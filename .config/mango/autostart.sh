...
systemctl --user unset-environment MANGOHUD >/dev/null 2>&1 || true
...
# portais xdg (file chooser, drag-and-drop, screenshare, etc)
systemctl --user restart xdg-desktop-portal-gtk xdg-desktop-portal >/dev/null 2>&1 &
